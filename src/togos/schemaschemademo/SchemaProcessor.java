...
public class SchemaProcessor {
	protected static Function<String, String> POSTGRES_CASIFIER = new Function<String, String>() {
		@Override
		public String apply(String phrase) throws RuntimeException {
			return phrase.replaceAll("_", "").replaceAll("::", "_")
					.toLowerCase().replaceAll("[^a-z0-9_]", "");
		}
	};

	static class ClassClassFilter<C, E extends Throwable> extends
			BaseStreamSource<C, E> implements
			StreamDestination<SchemaObject, E> {
		final Class<C> c;
	...
	static class TableClassFilter<E extends Throwable> extends
			BaseStreamSource<ComplexType, E> implements
			StreamDestination<SchemaObject, E> {
		@Override
		public void data(SchemaObject value) throws E {
			if (value instanceof ComplexType
					&& PropertyUtil.getFirstInheritedBoolean(value,
							Application.HAS_DB_TABLE, false)) {
		...
		for (Object obj : ns.items.values()) {
			if (obj instanceof SchemaObject) {
				...
				if (sobj.getLongName() != null) {
					...
					for (SchemaObject co : PropertyUtil.getAllInheritedValues(
							sobj, Core.COMMENT)) {
						if (commentString == "")
							commentString = " ; ";
	...
	static String USAGE_TEXT = "Usage: SchemaProcessor [options] [schema file]\n"
			+ "Options:\n"
	...
	public static void _main(String[] args) throws Exception {
		...
		for (int i = 0; i < args.length; ++i) {
			if ("-?".equals(args[i]) || "-h".equals(args[i])
				...
				outputCreateTablesScriptFile = new File(dir,
						"create-tables.sql");
		...
		sp.defineFieldModifier("index",
				SchemaInterpreter.FieldIndexModifierSpec.INSTANCE);
		sp.defineFieldModifier("enum", new SchemaInterpreter.EnumModifierSpec(
				Core.VALUE_TYPE));
		...
		ClassClassFilter<ComplexType, CompileError> complexTypeFilter = new ClassClassFilter<ComplexType, CompileError>(
				ComplexType.class);
		...
		if (outputCreateTablesScriptFile != null) {
			...
			final FileWriter createTablesWriter = new FileWriter(
					outputCreateTablesScriptFile);
			final SQLEmitter createTablesSqlEmitter = new SQLEmitter(
					createTablesWriter);
			final TableCreationSQLGenerator tcsg = new TableCreationSQLGenerator(
					createTablesSqlEmitter, tableNamer, columnNamer);
			tcsg.pipe(new StreamDestination<TableDefinition, CompileError>() {
				@Override
				public void data(TableDefinition td) throws CompileError {
					try {
		...
		if (outputDropTablesScriptFile != null) {
			...
			final FileWriter dropTablesWriter = new FileWriter(
					outputDropTablesScriptFile);
			final SQLEmitter dropTablesSqlEmitter = new SQLEmitter(
					dropTablesWriter);
			final TableCreationSQLGenerator tcsg = new TableCreationSQLGenerator(
					dropTablesSqlEmitter, tableNamer, columnNamer);
			tcsg.pipe(new StreamDestination<TableDefinition, CompileError>() {
				@Override
				public void data(TableDefinition td) throws CompileError {
		...
		if (outputSchemaRdfFile != null) {
			...
			final FileWriter rdfSchemaWriter = new FileWriter(
					outputSchemaRdfFile);
			final SchemaRDFGenerator srg = new SchemaRDFGenerator(
					rdfSchemaWriter, namespacePrefixes);
			sp.pipe(new StreamDestination<SchemaObject, CompileError>() {
				@Override
				public void data(SchemaObject value) throws CompileError {
					try {
						srg.data(value);
					} catch (IOException e) {
						throw new CompileError(e, BaseSourceLocation.NONE);
		...
		if (outputSchemaPhpFile != null) {
			...
			final FileWriter phpSchemaWriter = new FileWriter(
					outputSchemaPhpFile);
			final PHPSchemaDumper psd = new PHPSchemaDumper(phpSchemaWriter,
					phpSchemaClassNamespace);
			complexTypeFilter
					.pipe(new StreamDestination<ComplexType, CompileError>() {
						@Override
						public void data(ComplexType value) throws CompileError {
							try {
								psd.data(value);
							} catch (CompileError e) {
								throw e;
		...
		for (String sourceFilename : sourceFilenames) {
			...
			Reader sourceReader = isStdin ? new InputStreamReader(System.in)
			...
			try {
				...
				for (SourceLocation sLoc : e.getScriptTrace()) {
					System.err
							.println("  " + BaseSourceLocation.toString(sLoc));
				}
				e.printStackTrace();
				System.exit(1);
			} finally {
		...
		if (sourceFilenames.size() == 0) {
			System.err
					.println("Warning: Read no inputs.  Maybe you want to read from stdin by specifying '-'?");
		}
		if (!didSomething) {
			System.err
					.println("Warning: Didn't do anything.  Maybe you want to -o-<something>");
		}
	}
